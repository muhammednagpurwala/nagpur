...
<style>
    ...
    .metric-value { color: #0f172a; font-size: 1.45rem; font-weight: 750; margin-top: 5px; }
    .metric-unit { color: #64748b; font-size: .78rem; }
    .footer {
...
st.markdown("""
<div class="hero">
  <h1>⚙️ Belt Drive Transmission Design Checker</h1>
  <p>Interactive engineering calculator for Flat Belt and V-Belt transmission design.</p>
</div>
""", unsafe_allow_html=True)


st.markdown('<div class="section-title">Design Inputs</div>', unsafe_allow_html=True)
c1, c2 = st.columns(2)
...
if st.button("Calculate", type="primary", use_container_width=True):
    try:
        ...
        metrics = [
            ("Angle of Lap", f"{math.degrees(theta):.2f}°", ""),
            ("Belt Length", f"{L/1000:.3f}", "m"),
            ("Speed Ratio", f"{speed_ratio:.4f}", "N₂ / N₁"),
            ("Follower Speed", f"{N2:.1f}", "RPM"),
            ("Belt Speed", f"{v:.2f}", "m/s"),
            ("Maximum Power", f"{power_kw:.3f}", "kW"),
        ]
        cols = st.columns(3)
        for col, (label, value, unit) in zip(cols * 2, metrics):
            with col:
                st.markdown(
        ...
        with st.expander("Detailed tension results"):
            st.dataframe(
                [
        ...
        csv_data = (
            "Parameter,Value\n"
            f"Angle of Lap,{math.degrees(theta):.6f} deg\n"
            f"Belt Length,{L:.6f} mm\n"
            f"Speed Ratio,{speed_ratio:.6f}\n"
            f"Follower Speed,{N2:.6f} RPM\n"
            f"Belt Speed,{v:.6f} m/s\n"
            f"Centrifugal Tension,{Tc:.6f} N\n"
            f"Tight-Side Effective Tension,{T1:.6f} N\n"
            f"Slack-Side Tension,{T2:.6f} N\n"
            f"Maximum Power,{power_kw:.6f} kW\n"
        )
        st.download_button(
            "⬇️ Download Results (CSV)",
            data=csv_data,
            file_name="belt_drive_design_results.csv",
            mime="text/csv",
        )
...
st.markdown(
    '<div class="footer">Belt Drive Transmission Design Checker • Streamlit + Plotly</div>',
    unsafe_allow_html=True
)
